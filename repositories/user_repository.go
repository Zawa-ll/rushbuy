...
func NewUserRepository(table string, db *sql.DB) IUserRepository {
	return &UserManagerRepository{table, db} // returns the struct created
}
...
func (u *UserManagerRepository) Select(userName string) (user *datamodels.User, err error) {
	if userName == "" {
		return &datamodels.User{}, errors.New("The userName cannot be empty!")
	}
	...
	if len(result) == 0 {
		return &datamodels.User{}, errors.New("The user does not exist!")
	}
...
func (u *UserManagerRepository) SelectByID(userId int64) (user *datamodels.User, err error) {
	...
	user = &datamodels.User{}
	common.DataToStructByTagSql(result, user)
	return // return the user defined inside (user *datamodels.User, err error)
}
